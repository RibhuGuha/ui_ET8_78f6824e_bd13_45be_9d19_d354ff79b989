...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        ...
        // Add or Update Product MainImageTwo
        changes = TrackChanges('product');
        if (changes.length > 0 && changes.indexOf('MainImageTwo') > -1) {
            data = product.find((x) => x.key === 'MainImageTwo');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "Product_id", value: parseInt(productId) },
                    { key: "ProductMainImageTwo", value: newImageId }
                ];
                rslt = await Support.AddOrUpdateProduct(data, dropDownOptions);
                if (!rslt.status) return;

                let newValues = product.find((x) => x.key === 'MainImageTwo').value;
                newValues = { ...newValues, DocId: newImageId };
                row['product'].find((x) => x.key === 'MainImageTwo').value = newValues
                row['product'].find((x) => x.key === 'ProductMainImageTwo').value = newImageId;
                // Update Back for next tracking purpose
                UpdateBackUp('product');
